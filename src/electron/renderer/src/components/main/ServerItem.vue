...
<template>
  <details class="server-item-stage" open>
    <summary class="server-info-dock border-(--surface-soft) border-t border-b list-none forcusable ">
      <div class="server-info-state px-(--server-row-px) py-(--server-row-py) flex items-center gap-(--server-row-gap)">
        <div class="w-(--server-row-arrow-size) aspect-square fill-(--text-primary) flex justify-center items-center">
          <svg width="12" height="8" viewBox="0 0 12 8" xmlns="http://www.w3.org/2000/svg">
            <path d="M6 7.4L0 1.4L1.4 0L6 4.6L10.6 0L12 1.4L6 7.4Z" />
          </svg>
        </div>
        <div class="w-(--server-icon-size) aspect-square fill-(--primary) flex justify-center items-center">
    ...
    <div class="tasks-dock pl-(--task-row-pl)">
      <slot />
    </div>
  </details>
</template>
...
.server-item-stage{
    ...
    --task-row-pl: calc(var( --server-row-px) + var(--server-row-arrow-size) + var(--server-row-gap) + var(--server-icon-size) + var(--server-row-gap))
}
</style>
