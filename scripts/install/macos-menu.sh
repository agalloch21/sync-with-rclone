...
EXECUTABLE="${SYNC_WITH_RCLONE_EXECUTABLE:-}"
CANDIDATES=(
    "$INSTALL_DIR/sync-with-rclone.app/Contents/MacOS/sync-with-rclone"
...
EOF
    ...
    # Create workflows for each mode.
    ...
    rm -rf "$SERVICES_DIR/Sync with Rclone - 1 Push.workflow"
    rm -rf "$SERVICES_DIR/Sync with Rclone - 2 Pull.workflow"
    rm -rf "$SERVICES_DIR/Sync with Rclone - 3 Open Config.workflow"
