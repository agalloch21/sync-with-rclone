...
映射是用户创建和管理的持久同步关系，定义本地根目录、远端根目录及其专用同步过滤规则。创建映射不会让两个目录自动保持同步；每次同步仍由用户通过右键菜单、GUI 或 CLI 手动发起。
...
应用还提供独立于 `.gitignore` 的同步过滤规则。全局过滤规则作用于所有映射，映射过滤规则只作用于当前映射；两者取并集。匹配路径在本地与远端都不参与比较、复制和删除，也不会出现在确认列表中。同步过滤规则是双向硬排除，不支持 `!` 否定规则，默认过滤 `.DS_Store`、`Thumbs.db` 和 `.git`。

同步根路径本身可以是 symbolic link，应用会先解析为真实目录。同步根内部的 symbolic link 是同步边界：不会作为内容上传，也不会扫描或写入其目标；Pull 如果需要写入该 link 或其后代，必须在 review 前明确拒绝，并在执行前复检。
...
- [ ] GUI配置管理界面：管理映射和 rclone remote，并测试连接、SFTP `mtime` 保留能力和可选 hash 能力
