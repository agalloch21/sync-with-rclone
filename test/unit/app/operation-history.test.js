...
test('startSync records its failed result without shell presentation data', async () => {
  await withHistoryRuntime(async (appRoot) => {
    ...
    const diagnostics = await fs.readFile(path.join(appRoot, 'logs', 'diagnostics.log'), 'utf8')
    assert.match(diagnostics, /remoteFolderPath is required when bypassConfig is enabled/)
    assert.doesNotMatch(diagnostics, /operationId|sessionId/)
    assert.doesNotMatch(diagnostics, /^context:/m)
  })
})
...
test('startSync rejects an overlapping session before writing operation history', async () => {
  await withHistoryRuntime(async (appRoot) => {
    ...
    try {
      ...
      await assert.rejects(
        () => fs.access(path.join(appRoot, 'logs', 'diagnostics.log')),
        { code: 'ENOENT' },
      )
    }
    finally {
...
`, 'utf8')
    ...
    const diagnostics = await fs.readFile(path.join(runtimePaths.logDirectory, 'diagnostics.log'), 'utf8')
    assert.match(diagnostics, new RegExp(APP_ERROR_CODE.SYNC_REMOTE_SOURCE_NOT_FOUND))
    assert.match(diagnostics, /remote\.folder_not_found/)
    assert.match(diagnostics, /directory not found/)
    assert.doesNotMatch(diagnostics, /^context:/m)
  })
})
