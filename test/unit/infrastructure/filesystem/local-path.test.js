...
import { createTemporaryDirectory } from '#test/helpers/temporary-files.js'
...
test('resolveLocalDirectoryPath validates and resolves local directory input', async (t) => {
  const temporaryPath = await createTemporaryDirectory(t, 'local-path-validation-')
  const targetDirectory = path.join(temporaryPath, 'nested')
  const filePath = path.join(temporaryPath, '.gitignore')
  ...
  const targetPath = (await fs.realpath(targetDirectory)).replaceAll(path.sep, path.posix.sep)
  const relativePath = path.relative(pwdPath, targetDirectory)
  assert.equal(resolveLocalDirectoryPath(relativePath), targetPath)
  assert.equal(resolveLocalDirectoryPath(`${relativePath}/`), targetPath)
  assert.equal(resolveLocalDirectoryPath(`${relativePath}\\`), targetPath)

  assert.throws(() => resolveLocalDirectoryPath(path.join(temporaryPath, 'missing')), {
    name: 'InfrastructureError',
    code: INFRASTRUCTURE_ERROR_CODE.PATH_NOT_FOUND,
  })
  assert.throws(() => resolveLocalDirectoryPath(filePath), {
    name: 'InfrastructureError',
    code: INFRASTRUCTURE_ERROR_CODE.PATH_NOT_DIRECTORY,
  })
})

test('resolveLocalDirectoryPath preserves absolute Windows-style paths', async (t) => {
  const absoluteWindowsPath = await createTemporaryDirectory(t, 'local-path-absolute-')
  assert.equal(
    resolveLocalDirectoryPath(absoluteWindowsPath),
    (await fs.realpath(absoluteWindowsPath)).replaceAll(path.sep, path.posix.sep),
  )
})
