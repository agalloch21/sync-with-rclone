...
import { createMapping, createServer, getServer, listGlobalExclusionPatterns, listServers, updateMapping, updateMappingExclusionPatterns, updateServer } from '#src/app/app-api.js'
...
export function createFormModalHandlers() {
  ...
  async function getGlobalExclusionPatternsHandler(_event) {
    try {
      return toSuccessfulResult(await listGlobalExclusionPatterns())
    }
    catch (error) {
      return await reportRequestError(error)
    }
  ...
  async function updateMappingExclusionPatternsHandler(_event, payload) {
    try {
      assertPayloadObject(payload)
    }
    catch (error) {
    ...
    return await runReportedOperation(
      ...
      onProgress => updateMappingExclusionPatterns(payload.mapping, payload.exclusionPatterns, onProgress),
    )
  }
  ...
  return {
    ...
    updateMappingExclusionPatternsHandler,
