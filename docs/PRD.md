...
映射是用户创建和管理的持久同步关系，定义本地根目录、远端根目录及其专用规则。创建映射不会让两个目录自动保持同步；每次同步仍由用户通过右键菜单、GUI 或 CLI 手动发起。
...
`.gitignore`、全局规则和 mapping 规则只描述本地工作区不应上传的内容。`Push` 会让远端与过滤后的本地内容一致，因此删除远端多余路径；`Pull` 以完整远端内容为 truth，不应用本地 ignore 规则，并可恢复本地被忽略但远端仍存在的内容。
