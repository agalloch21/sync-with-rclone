...
import { ensureAppConfig, loadAppConfig, loadAppGlobalExclusionPatterns, loadAppMappings } from '#src/infrastructure/configuration/app-config-store.js'
...
test('loadAppConfig reads and normalizes config', async () => {
  ...
  assert.deepEqual(config.globalExclusionPatterns, ['.DS_Store'])
  assert.equal(config.mappings[0].displayName, 'Projects')
  assert.equal(config.mappings[0].rcloneRemote, 'synology')
  assert.deepEqual(config.mappings[0].exclusionPatterns, ['node_modules/'])
...
test('loadAppConfig allows an empty remoteBasePath for syncing to the remote root', async () => {
  ...
  assert.deepEqual(config.globalExclusionPatterns, ['.DS_Store', 'Thumbs.db', '.git'])
})
...
test('loadAppGlobalExclusionPatterns does not validate mappings', async () => {
  ...
  assert.deepEqual(
    await loadAppGlobalExclusionPatterns(configPath),
    ['.DS_Store', 'Thumbs.db'],
  )
})

test('loadAppMappings does not validate global exclusion patterns', async () => {
...
test('loadAppGlobalExclusionPatterns validates its own projection', async () => {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'sync-with-rclone-global-patterns-invalid-'))
  const configPath = path.join(tempDir, 'config.json')

  await fs.writeFile(configPath, JSON.stringify({
    globalExclusionPatterns: ['valid', null],
    mappings: [],
  }))

  await assert.rejects(
    () => loadAppGlobalExclusionPatterns(configPath),
    {
      name: 'InfrastructureError',
      code: INFRASTRUCTURE_ERROR_CODE.CONFIG_LOAD_FAILED,
    },
...
test('ensureAppConfig creates a default config when config is missing', async () => {
  ...
  assert.deepEqual(JSON.parse(await fs.readFile(configPath, 'utf8')), {
    globalExclusionPatterns: ['.DS_Store', 'Thumbs.db', '.git'],
    mappings: [],
  })
})
...
test('ensureAppConfig does not overwrite an existing config', async () => {
  ...
  await fs.writeFile(configPath, JSON.stringify({ globalExclusionPatterns: ['keep'], mappings: [] }))

  const result = await ensureAppConfig({ configDirectory, configPath })

  assert.deepEqual(result, { configCreated: false, configPath })
  assert.deepEqual(JSON.parse(await fs.readFile(configPath, 'utf8')), {
    globalExclusionPatterns: ['keep'],
    mappings: [],
  })
})
